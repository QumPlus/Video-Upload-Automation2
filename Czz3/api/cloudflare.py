    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            if not self.api_token or not self.api_token.strip():
                print("CloudFlare API: API token is empty or missing")
                return False
                
            if not self.account_id or not self.account_id.strip():
                print("CloudFlare API: Account ID is empty or missing")
                return False
            
            # Test API connection
            url = f"{self.base_url}/accounts/{self.account_id}/stream"
            print(f"CloudFlare API: Testing connection to {url}")
            
            response = requests.get(url, headers=self.headers, timeout=10)
            
            print(f"CloudFlare API: Response status code: {response.status_code}")
            
            if response.status_code == 200:
                print("CloudFlare API: Connection successful!")
                return True
            elif response.status_code == 401:
                print("CloudFlare API: Invalid API token - check your credentials")
                try:
                    error_data = response.json()
                    print(f"CloudFlare API: Error details: {error_data}")
                except:
                    print(f"CloudFlare API: Response text: {response.text}")
                return False
            elif response.status_code == 403:
                print("CloudFlare API: Access forbidden - check account permissions")
                try:
                    error_data = response.json()
                    print(f"CloudFlare API: Error details: {error_data}")
                except:
                ...
                print("CloudFlare API: Account ID not found - check your account ID")
                return False
            else:
            ...
            print(f"CloudFlare API: Request timeout - check your internet connection: {e}")
            return False
        except requests.exceptions.ConnectionError as e:
            print(f"CloudFlare API: Connection error - check your internet connection: {e}")
            return False
        except requests.exceptions.RequestException as e:
            print(f"CloudFlare API: Request error: {e}")
            return False
        except Exception as e:
            print(f"CloudFlare API: Unexpected error: {e}")
            return False
